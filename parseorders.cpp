...
{
  int moveDir = 0; // Направлене 0 - стоим 1 - к донору 2 - к получателю
  ...
  if ((myLoc->region->xloc == tLoc->region->xloc)&&
	...
	{ // Да мы рядом с донором !!!
		...
		if (amt > fLim)
			if( !pCheck )
			{
				...
				moveDir = 2;
			} // Все что имел донор получили потопали к получателю
	}
	if (moveDir==0)// Похоже мы где то посередине и никуда не собираемся
	{
	 if (u->items.GetNum(item) > myLim)
	 { // Нести есть чего топаем к получателю
		moveDir = 2;
	 }
	 else
	 { // Нести не чего топаем к донору
		moveDir = 1;
	 }
	}


	if(moveDir != 0)  // Надо топать
	{           // Начали топать
	if (u->object->IsBoat()) 
	{
		SailOrder *m = 0;
	...
	{
		MoveOrder *m = 0;		// Тупо скопировано из ProcessMoveOrder
		if( pCheck )			// Учебная тревога
			{
	...
	}							// Конец тупо скопированного фрагмента
		...
		while ((xMove!=0)||(yMove!=0))
		{
			...
			// Цель на западе идем через север
			if ((xMove < 0)&&(yMove ==0)&&(fm==0)) { ++xMove; ++yMove; d=5; ++fm;}
			// Цель на востоке идем через юг
			if ((xMove > 0)&&(yMove ==0)&&(fm==0)) { --xMove; --yMove; d=2; ++fm;}
			// Цель на северозападе
			if ((xMove < 0)&&(yMove <0)&&(fm==0))  { ++xMove; ++yMove; d=5; ++fm;}
			// Цель на северовостоке
	...
	}  // Кончили топать по боевому расписанию
  }
}
