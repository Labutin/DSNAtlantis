...
int ARegion::CountConnectingRoads()
{
    ...
    forlist (&objects)
    {
        ...
        if (o->IsRoadUsable())
        {
            for (int i = 0; i < NDIRS; i++)
            {
//                if (neighbors[i]->HasExitRoad(GetRealDirComp(i)))
		if (neighbors[i] && neighbors[i]->HasExitRoad(GetRealDirComp(i)))
                    connections ++;
            }
        }
    }
            ...
            forlist(&objects) {
                ...
                forlist(&o->units) {
                    ...
                    if (!sawgate && (u->faction == fac &&
                        u->GetSkill(S_GATE_LORE))||((fac->IsNPC())))
//                    if (!sawgate && u->faction == fac &&
...
// dsnlab
            if (farsight && farsight->unit)
              if (farsight->unit->GetSkill(S_MIND_READING) > 2)
              {
                detfac = 1;
              }
        }
...
int ARegion::IsGuarded1()
{
    forlist (&objects) {
        Object * o = (Object *) elem;
        forlist (&o->units) {
            Unit * u = (Unit *) elem;
            if (u->guard == GUARD_GUARD) return u->faction->num;
        }
    }
    return 0;
}
