                ...
                do
                {
                    if( ItemDefs[ it->type ].type & IT_MAN )
                    {
                        soldiers[x++] = new Soldier( u,
                                                     ...
                                                     ass );
                        // 2000/MAR/16 LLS
                        // Clear next element, and hopefully avoid
                        // seg fault.
//                        if( !(it->next) )
//                        {
//                            _elem2 = NULL;
...
//    if (ItemDefs[thespoil].baseprice >= 1000000)
//    {
//        return;
//    }
...
void Army::DoHealLevel( Battle *b, int type, int rate, int useItems )
{
    for (int i=0; i<NumAlive(); i++)
    {
        ...
        while (s->healing)
        {
            ...
            if (temp->canbehealed)
            {
//	        b->AddLine (*(temp->unit->name) + " try heal");
                s->healing--;
